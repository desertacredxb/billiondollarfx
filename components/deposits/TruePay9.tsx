  ...
  const handleSubmit = async (event: React.FormEvent) => {
    ...
    try {
      ...
      const paymentUrl =
        ...
        response.data.data?.pay ??
        response.data.decrypted?.url;
      ...
      console.log("TruePay9 deposit failed:", error);
    ...
    <div className="flex flex-col gap-4">
      <div className="max-w-md border border-gray-700 bg-[#111827] rounded-2xl shadow-lg p-6 flex flex-col space-y-4">
        <div className="flex justify-between items-center">
          ...
          <h2 className="text-xl font-semibold">TruePay9</h2>
        </div>
        <p className="text-gray-300 text-sm">
          Secure and fast deposit using TruePay9. Click below to proceed.
        </p>
        <Button
          text="Deposit"
          onClick={() => {
      ...
      {showModal && (
        <div className="fixed inset-0 flex items-center justify-center bg-black/60 z-50 px-4">
          <div className="bg-[#1f2937] p-6 rounded-xl w-full max-w-md relative">
            ...
            <h2 className="text-xl font-bold mb-4">Deposit with TruePay9</h2>
            <form onSubmit={handleSubmit} className="space-y-4">
              ...
              <Button
                ...
                disabled={loading || !form.accountNo}
              />
            </form>
          </div>
        </div>
